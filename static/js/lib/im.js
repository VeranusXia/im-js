...
IMService.MSG_SUBSCRIBE_GROUP = 133
...
IMService.prototype.onMessage = function (data) {
    ...
    if (cmd == IMService.MSG_IM) {
        ...
        msg.content = IMService.Utf8ArrayToStr(new Uint8Array(data, IMService.HEADSIZE + 16, len-16));

        console.log("im message sender:" + msg.sender +" roomid:" + msg.roomid + "content:" + msg.content);
        ...
        console.log("im message sender:" + msg.sender +" groupid:" + msg.groupid + "content:" + msg.content);
...
IMService.prototype.enterGroup = function(groupid){
	if (this.connectState != IMService.STATE_CONNECTED) {
        return false;
    }
    ...
    this.send(IMService.MSG_SUBSCRIBE_GROUP, buf);
}
IMService.prototype.levelRoom = function(roomid){
	if (this.connectState != IMService.STATE_CONNECTED) {
        return false;
    }
	var arrayBuffer = new ArrayBuffer(8);
	var buf =new Uint8Array(arrayBuffer);
	hton64(buf,0,roomid);
	var r =this.send(IMService.MSG_LEAVE_ROOM,buf)
	if (!r) {
        return false;
    }
...
IMService.prototype.sendRoomMessage = function (msg) {
    ...
    var content = IMService.StrToUtf8Array(msg.content);
    var arrayBuffer = new ArrayBuffer(16+content.length);
    var buf = new Uint8Array(arrayBuffer);
    var pos = 0;
